...
export async function getCategories(
  appContext: ContextStore
): Promise<CategoryHierarchyFacetResultCategoryNode[]> {
  ...
  const builder = new ProductSearchBuilder(appContext.getDefaultSettings())
    ...
    .facets((f) =>
      f.addProductCategoryHierarchyFacet("ImmediateParent", null, {
        displayName: true
      })
    );
  ...
  return appContext
    ...
    .then((response) => {
      ...
      const categoryFacetResult = response.facets
        .items[0] as CategoryHierarchyFacetResult;
      return categoryFacetResult.nodes
        .filter((node) => node.category.displayName)
        .sort(
          (a, b) =>
            a.category.displayName?.localeCompare(
