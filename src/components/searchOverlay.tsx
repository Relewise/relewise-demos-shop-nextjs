...
import { ProductRecommendationResponse, ProductSearchBuilder, ProductSearchResponse, SearchCollectionBuilder, SearchTermBasedProductRecommendationBuilder, SearchTermPredictionBuilder, SearchTermPredictionResponse, SearchTermPredictionResult } from "@relewise/client";
...
const Component = (props: SearchOverlayProps) => {
    ...
    const [fallbackProducts, setFallbackProducts] = React.useState<ProductRecommendationResponse | undefined>()
    ...
    useEffect(() => {
        ...
        const searchCollectionBuilder = new SearchCollectionBuilder()
            .addRequest(new ProductSearchBuilder(contextStore.getDefaultSettings())
                ...
                .build())
            .addRequest(new SearchTermPredictionBuilder(contextStore.getDefaultSettings())
        ...
        searcher.batch(searchCollectionBuilder.build()).then(response => {
            if (response && response.responses) {
                ...
                if (productResult.hits < 1) {
                    const searchTermBasedProductRecommendationBuilder = new SearchTermBasedProductRecommendationBuilder(contextStore.getDefaultSettings())
                        .setSelectedProductProperties(contextStore.getProductSettings())
                        .setSelectedVariantProperties({ allData: true })
                        .setTerm(props.input)
                    ...
                    recommender.recommendSearchTermBasedProducts(searchTermBasedProductRecommendationBuilder.build())
                        .then(response => {
    ...
    return (isSearching() && document != undefined) ? createPortal(
        <>
            {products &&
                <div className="modal">
                        ...
                        <div className="flex gap-3">
                                ...
                                {predictions.length > 0 &&
                                    <div className="p-3 bg-white mb-3">
                                        ...
                                        {predictions.map((prediction, index) =>
                                            <a className="mb-1 block cursor-pointer" key={index}
                                                onClick={() => {
                            ...
                            <div className="w-4/5">
                                <div className="flex gap-6 items-end p-3 bg-white rounded mb-3">
                                    ...
                                    <span v-if="result.hits > 0">Showing {page * 30 - 29} - {products?.hits < 30 ? products?.hits : page * 30} of {products?.hits}</span>
                                </div>
                                {(products.redirects && products.redirects.length > 0) &&
                                    <div className="mb-3 p-3 bg-white">
                                        ...
                                        {products.redirects?.map((redirect, index) =>
                                            <div key={index} className="mb-1 pb-1 flex border-b border-solid border-gray-300">
                                ...
                                {(fallbackProducts && fallbackProducts.recommendations && fallbackProducts.recommendations.length > 0) &&
